...
int main()
{
	...
	octo_dict_cll_t *test_cll = octo_cll_init(8, 64, 128, 1, init_master_key);
	...
	test_cll = octo_cll_rehash(test_cll, test_cll->keylen, test_cll->vallen, 1, 1, new_master_key);
	octo_cll_stats_msg(test_cll);
	printf("test_cll: Poking inserted records...\n");
	if(!(octo_cll_poke(key1, (const octo_dict_cll_t *)test_cll)))
	{
	...
	test_cll = octo_cll_rehash_safe(test_cll, test_cll->keylen, test_cll->vallen, 4096, 3, new_master_key);
	octo_cll_stats_msg(test_cll);
	printf("test_cll: Poking inserted records...\n");
	if(!(octo_cll_poke(key1, (const octo_dict_cll_t *)test_cll)))
	{
